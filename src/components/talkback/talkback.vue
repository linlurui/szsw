<template>
  <div class="content">
    ...
    <span class="titleName" v-if="$exe.installation">{{titleName}}</span>
    <!-- 左侧视频列表 -->
        ...
        <transition name="fade">
          <ul class="child" v-show="item0.areaName == lifeName">
            <li v-for="item in item0.projectList" :key="item.pid" :title="item.projectName">
              <div
                class="lists"
                :class="item.pid == liId ? 'active' : ''"
                @click="openVideo(item.pid)"
              >
        ...
        >
          ...
          <div class="video-only" v-if="videoNum==1">
            ...
            <span
              ...
              v-if="!playback&&(!beginTime||!endTime)"
            >{{cutDefinition?'高清':'标清'}}</span>
            <span
              class="cut"
              @click="checkedTime"
              v-if="beginTime&&endTime"
              :style="{right: playback?'0.6rem':'0.5rem'}"
            >确定</span>
            <span class="cut-back" @click="playback=false,getIframeSrc()" v-if="playback">返回</span>
            <span class="center"></span>
            <iframe :src="iframeSrc" height="100%" width="100%" id="player17" allowfullscreen></iframe>
          </div>
          <!-- 根据不同的数值循环出显示video -->
          <div
            class="area-list"
...
export default {
  ...
  methods: {
    ...
    getVideo(get = true) {
      ...
      this.$axios
        .post(`/api/ProjectVideoAreaApi/getVideoListJT?cid=${this.cid}`)
        .then(res => {
          this.listWrap = res.data;
          if (get == true) {
            this.getEveryProjectFirstVideo(this.listWrap);
          }
        });
    },
    ...
    talkback() {
      this.$axios
        .post(`/api/sbGroupTalkback/getAccountList?cpid=${this.cid}&isIdType=1`)
        .then(res => {
    ...
    getImgList() {
      ...
      this.$axios
        .post(`/api/ProjectVideoAreaApi/getVideoListImgUrl?cid=${this.cid}`)
        .then(res => {
    ...
    getDownloadVideo() {
      ...
      this.$axios
        .post(
          `/api/sbAccountTalkback/getAccountListPage?isIdType=1&cpid=${this.cid}&atName=${this.searchName}&pageSize=7&pageNum=${this.pageNum}`
        )
        .then(res => {
          if (res.data.code == 0) {
    ...
    getTitle() {
      this.$axios
        .post(`/api/groupTitleApi/getTitle?cid=${this.cid}`)
        .then(res => {
          if (res.data.code == 0) {
    ...
    maximize(e) {
      ...
      this.$nextTick(() => {
        ...
        for (let i = 0; i < this.urlList.length; i++) {
          if (
            this.urlList[i].videoName == name &&
            this.urlList[i].isControl == 1
          ) {
            this.sn = this.urlList[i].videoSn;
            this.pid = this.urlList[i].pid;
          } else if (this.urlList[i].videoName == name) {
    ...
    getMap() {
      this.map = new AMap.Map("mapId", {
        ...
        center: [
          Number(
            this.videoList.filter(item => item.longitude != null && item.longitude != "")[0]
              .longitude
          ),
          Number(
            this.videoList.filter(item => item.latitude != null && item.latitude != "")[0]
              .latitude
          )
        ],
    ...
    editMark(add) {
      this.$axios
        .post(
          `/api/ProjectVideo/updateVideoCoordinate?videoSn=${add == 'add' ? this.addMarkSn : this.editMarkSn}&longitude=${this.draggingLng}&latitude=${this.draggingLat}`
        )
        .then(res => {
          if (res.data.code == 0) {
    ...
    downloadVideoList() {
      ...
      this.allDownlodList = {};
      // 下载
      location.href = `http://47.106.71.3:8080/api/sbGroupTalkback/ftpDownload?ftpPath=${
        this.accountId
      }&user=${user}&name=${name}&startTime=${this.downloadTime[0]}&endTime=${
        this.downloadTime[1]
      }`;
    }
  },
